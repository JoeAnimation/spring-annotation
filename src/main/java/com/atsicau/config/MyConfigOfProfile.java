 ...
 * 数据源：(/A)(/B)(/C)
 * 
 * @Profile:指定组件在哪个环境的情况下才能被注册到容器中，不指定，任何环境下都能注册组件
 * 
 * 1)加了环境标识的bean,只有这个环境被激活的时候才能被注册到容器中，默认是default
...
public class MyConfigOfProfile implements EmbeddedValueResolverAware{
	...
	@Profile("dev")
	@Bean("devDataSource")
	public DataSource dataSourceDev(@Value("${dnb.password}") String pwd) throws PropertyVetoException{
		ComboPooledDataSource dataSource = new ComboPooledDataSource();
		dataSource.setUser(user);
		dataSource.setPassword(pwd);
